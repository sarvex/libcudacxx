...
_LIBCUDACXX_BEGIN_NAMESPACE_CPO(__ssize)
#if _LIBCUDACXX_STD_VER > 17
  template<class _Tp>
  concept __can_ssize_ptrdiff = requires (_Tp&& __t) {
    typename make_signed_t<decltype(_CUDA_VRANGES::size(__t))>;
    requires sizeof(ptrdiff_t) > sizeof(make_signed_t<decltype(_CUDA_VRANGES::size(__t))>);
    { static_cast<ptrdiff_t>(_CUDA_VRANGES::size(__t)) } -> integral;
  };

  template<class _Tp>
  concept __can_ssize_signed = requires (_Tp&& __t) {
    typename make_signed_t<decltype(_CUDA_VRANGES::size(__t))>;
    requires sizeof(ptrdiff_t) <= sizeof(make_signed_t<decltype(_CUDA_VRANGES::size(__t))>);
    { static_cast<make_signed_t<decltype(_CUDA_VRANGES::size(__t))>>(_CUDA_VRANGES::size(__t)) } -> integral;
  };
#else // ^^^ CXX20 ^^^ / vvv CXX17 vvv
  template<class _Tp>
  _LIBCUDACXX_CONCEPT_FRAGMENT(
    __can_ssize_ptrdiff_,
    requires(_Tp&& __t)(
      typename(make_signed_t<decltype(_CUDA_VRANGES::size(__t))>),
      requires((sizeof(ptrdiff_t) > sizeof(make_signed_t<decltype(_CUDA_VRANGES::size(__t))>))),
      (static_cast<ptrdiff_t>(_CUDA_VRANGES::size(__t)))
    ));

  template<class _Tp>
  _LIBCUDACXX_CONCEPT __can_ssize_ptrdiff = _LIBCUDACXX_FRAGMENT(__can_ssize_ptrdiff_, _Tp);
  ...
  _LIBCUDACXX_CONCEPT_FRAGMENT(
    ...
    requires(_Tp&& __t)(
      typename(make_signed_t<decltype(_CUDA_VRANGES::size(__t))>),
      requires(sizeof(ptrdiff_t) <= sizeof(make_signed_t<decltype(_CUDA_VRANGES::size(__t))>)),
      requires(integral<make_signed_t<decltype(_CUDA_VRANGES::size(__t))>>),
      (static_cast<make_signed_t<decltype(_CUDA_VRANGES::size(__t))>>(_CUDA_VRANGES::size(__t)))
    ));

  template<class _Tp>
  _LIBCUDACXX_CONCEPT __can_ssize_signed = _LIBCUDACXX_FRAGMENT(__can_ssize_signed_, _Tp);
#endif // _LIBCUDACXX_STD_VER < 20

  struct __fn {
    _LIBCUDACXX_TEMPLATE(class _Tp)
      (requires __can_ssize_ptrdiff<_Tp>)
    _LIBCUDACXX_NODISCARD_ATTRIBUTE _LIBCUDACXX_INLINE_VISIBILITY
    constexpr auto operator()(_Tp&& __t) const
    ...
    _LIBCUDACXX_TEMPLATE(class _Tp)
      (requires __can_ssize_signed<_Tp>)
    _LIBCUDACXX_NODISCARD_ATTRIBUTE _LIBCUDACXX_INLINE_VISIBILITY
    constexpr auto operator()(_Tp&& __t) const
      noexcept(noexcept(_CUDA_VRANGES::size(__t))) {
