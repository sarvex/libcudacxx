...
_LIBCUDACXX_BEGIN_NAMESPACE_CPO(__begin)
  ...
  struct __fn {
    ...
    _LIBCUDACXX_NODISCARD_ATTRIBUTE _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Tp (&__t)[]) const noexcept
    {
      return __t + 0;
    }
    ...
    _LIBCUDACXX_TEMPLATE(class _Tp)
      (requires __member_begin<_Tp>)
    _LIBCUDACXX_NODISCARD_ATTRIBUTE _LIBCUDACXX_INLINE_VISIBILITY constexpr auto operator()(_Tp&& __t) const
  ...
  _LIBCUDACXX_TEMPLATE(class _Tp)
      (requires (!is_array_v<_Tp> && !__member_begin<_Tp> && !__unqualified_begin<_Tp>))
...
template<class _Tp>
  ...
  _LIBCUDACXX_TEMPLATE(class _Tp)
      (requires (!is_array_v<_Tp> && !__member_end<_Tp> && !__unqualified_end<_Tp>))
    void operator()(_Tp&&) const = delete;
  };
_LIBCUDACXX_END_NAMESPACE_CPO
