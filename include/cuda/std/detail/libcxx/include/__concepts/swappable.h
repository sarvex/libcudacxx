...
#ifndef __cuda_std__
#include <__concepts/__concepts_macros.h>
...
#pragma nv_diag_suppress 2642
  ...
  template<class _Tp, class _Up, size_t _Size>
  concept __noexcept_swappable_arrays = __swappable_arrays<_Tp, _Up, _Size> &&
    requires(_Tp& __t, _Up& __u, const __fn& __swap) {
      noexcept(__swap(__t, __u));
    };    
#pragma nv_diag_default 2642
  ...
  struct __fn {
    ...
    _LIBCUDACXX_INLINE_VISIBILITY constexpr void operator()(_Tp(& __t)[_Size], _Up(& __u)[_Size]) const
      noexcept(__noexcept_swappable_arrays<_Tp, _Up, _Size>)
    {
...
template<class _Tp, class _Up, class _Size>
  ...
  template<class _Tp, class _Up, size_t _Size>
  _LIBCUDACXX_INLINE_VAR constexpr bool __swappable_arrays<_Tp, _Up, _Size, void_t<int>> =
    _LIBCUDACXX_FRAGMENT(__swappable_arrays_, _Tp, _Up, _CUDA_VSTD::integral_constant<size_t, _Size>);

  template<class _Tp, class _Up>
  _LIBCUDACXX_INLINE_VAR constexpr bool __noexcept_swappable = 
    noexcept(__fn{}(_CUDA_VSTD::declval<_Tp>(), _CUDA_VSTD::declval<_Up>()));
  
  template<class _Tp, class _Up, class _Size>
  _LIBCUDACXX_CONCEPT_FRAGMENT(
    __noexcept_swappable_arrays_,
    requires(_Tp(& __t)[_Size::value], _Up(& __u)[_Size::value], const __fn& __swap)(
      requires(__swappable_arrays<_Tp, _Up, _Size::value>),
      requires(__noexcept_swappable<decltype(__t[0]), decltype(__u[0])>)
    ));
  
  template<class _Tp, class _Up, size_t _Size>
  _LIBCUDACXX_INLINE_VAR constexpr bool __noexcept_swappable_arrays<_Tp, _Up, _Size, void_t<int>> = 
    _LIBCUDACXX_FRAGMENT(__noexcept_swappable_arrays_, _Tp, _Up, _CUDA_VSTD::integral_constant<size_t, _Size>);
#endif // _LIBCUDACXX_STD_VER < 20
