...
struct convertible_forward_sized_iterator {
  ...
  __host__ __device__ constexpr convertible_forward_sized_iterator operator++(int) { return forward_sized_iterator(it_++); }
...
__host__ __device__ constexpr bool test() {
...

  {
    ...
    cuda::std::ranges::zip_view v(cuda::std::views::iota(0), SizedRandomAccessView(buffer1));
    static_assert(!cuda::std::ranges::common_range<decltype(v)>);
    static_assert(!SentinelHasMinus<decltype(v)>);
  }
