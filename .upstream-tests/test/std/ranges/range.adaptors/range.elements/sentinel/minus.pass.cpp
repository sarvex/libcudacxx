...
struct SizedSent {
  ...
  __host__ __device__ friend constexpr auto operator-(const SizedSent& st, const Iter<Const>& it) { return st.end_ - it.it_; }

  __host__ __device__ friend constexpr auto operator-(const Iter<Const>& it, const SizedSent& st) { return it.it_ - st.end_; }
};
...
struct CrossSizedSent {
  ...
  template <bool C>
  __host__ __device__  friend constexpr auto operator-(const CrossSizedSent& st, const Iter<C>& it) {
    return st.end_ - it.it_;
  }

  template <bool C>
  __host__ __device__  friend constexpr auto operator-(const Iter<C>& it, const CrossSizedSent& st) {
...
template <class BaseRange>
using EleConstIter = cuda::std::ranges::iterator_t<const ElementsView<BaseRange>>;

template <class BaseRange>
using EleSent = cuda::std::ranges::sentinel_t<ElementsView<BaseRange>>;

template <class BaseRange>
using EleConstSent = cuda::std::ranges::sentinel_t<const ElementsView<BaseRange>>;
