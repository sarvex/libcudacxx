...
__host__ __device__ constexpr void testOne() {
  ...
  if (!__builtin_is_constant_evaluated()) {
    ...
    cuda::std::ranges::drop_while_view dwv{cuda::std::move(range), always<false>{}};
  ...
  // 1 element dropped
  {
    int buffer[] = {1};
    auto range   = make_subrange<Range, Iter, Sent>(buffer);
    cuda::std::ranges::drop_while_view dwv{cuda::std::move(range), always<true>{}};
    decltype(auto) it = dwv.begin();
    static_assert(cuda::std::same_as<decltype(it), Iter>);
  ...
  // multiple elements. some elements dropped
  {
    ...
    auto range   = make_subrange<Range, Iter, Sent>(buffer);
    cuda::std::ranges::drop_while_view dwv{cuda::std::move(range), LessThan3{}};
    decltype(auto) it = dwv.begin();
    static_assert(cuda::std::same_as<decltype(it), Iter>);
  ...
  // Make sure we do not make a copy of the predicate when we call begin()
  {
    ...
    auto range = make_subrange<Range, Iter, Sent>(buffer);
    cuda::std::ranges::drop_while_view dwv{cuda::std::move(range), TrackingPred(&moved, &copied)};
    moved                    = false;
    copied                   = false;
    auto it = dwv.begin();
    assert(!moved);
  ...
  // Test with a non-const predicate
  {
    int buffer[] = {1, 2, 3, 2, 1};
    auto range   = make_subrange<Range, Iter, Sent>(buffer);
    cuda::std::ranges::drop_while_view dwv{cuda::std::move(range), [](int i) mutable { return i < 3; }};
    decltype(auto) it = dwv.begin();
    static_assert(cuda::std::same_as<decltype(it), Iter>);
    assert(base(it) == buffer + 2);
  }
