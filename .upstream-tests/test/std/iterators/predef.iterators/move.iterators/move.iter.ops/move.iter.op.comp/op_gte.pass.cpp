...
struct CustomIt {
  ...
  CustomIt() = default;
  __host__ __device__ TEST_CONSTEXPR_CXX17 explicit CustomIt(int* p) : p_(p) {}
  __host__ __device__ int& operator*() const;
  __host__ __device__ CustomIt& operator++();
  __host__ __device__ CustomIt operator++(int);
  __host__ __device__ TEST_CONSTEXPR_CXX17 friend bool operator>=(const CustomIt& a, const CustomIt& b) { return a.p_ >= b.p_; }
  int *p_ = nullptr;
};

template <class It>
__host__ __device__ TEST_CONSTEXPR_CXX17 void test_one()
{
  int a[] = {3, 1, 4};
