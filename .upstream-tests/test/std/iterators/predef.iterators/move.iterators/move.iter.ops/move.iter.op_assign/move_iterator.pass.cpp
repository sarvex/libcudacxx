...
struct ToIter {
    ...
    __host__ __device__ explicit TEST_CONSTEXPR_CXX17 ToIter() : m_value(0) {}
    __host__ __device__ TEST_CONSTEXPR_CXX17 ToIter(const ToIter &src) : m_value(src.m_value) {}
    // Intentionally not defined, must not be called.
    __host__ __device__ ToIter(char *src);
    __host__ __device__ TEST_CONSTEXPR_CXX17 ToIter &operator=(char *src) {
        m_value = src;
        return *this;
    }
    __host__ __device__ TEST_CONSTEXPR_CXX17 ToIter &operator=(const ToIter &src) {
        m_value = src.m_value;
        return *this;
    }
    char *m_value;
...
__host__ __device__ TEST_CONSTEXPR_CXX17 bool test_conv_assign()
{
    char c = '\0';
    char *fi = &c;
